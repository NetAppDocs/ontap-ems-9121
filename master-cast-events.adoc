...
[.lead]
master.cast EMS events occur when the master service on a node cannot connect to the current cluster master and casts a vote against it in the global database.
