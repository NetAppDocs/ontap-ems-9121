...
[.lead]
dbs.file EMS events occur when ONTAP encounters issues associated with a mounted container file system on a node, such as low capacity or a read-only state.
