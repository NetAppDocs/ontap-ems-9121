...
[.lead]
oss.monitor EMS events occur when ONTAP encounters issues associated with the ONTAP(R) Status Service (OSS) monitor and its SNMP-based device monitoring plug-ins.
